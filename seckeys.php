...
$sql = "SELECT * FROM `vetcliniccorp`.`seckeys` WHERE `emplnumber` = $editempnum and `sequence` = 1;";
...
echo " >Not Assigned</td></tr>";
$sql = "SELECT * FROM `vetcliniccorp`.`seckeys` WHERE `emplnumber` = $editempnum and `sequence` = 2;";
$result = $mysqli->query($sql);
$row_cnt = $result->num_rows;
$row = $result->fetch_row();
