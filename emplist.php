...
{
	case "01":
		$sql1 = "SELECT emplnumber, lname, fname, address, address2, city, state, zipcode FROM vetcliniccorp.employee WHERE status = \"A\" ORDER BY lname, fname";
		break;
	case "02":
		$sql1 = "SELECT emplnumber, lname, fname, address, address2, city, state, zipcode FROM vetcliniccorp.employee WHERE status = \"I\" ORDER BY lname, fname";
		break;
	case "03":
		$sql1 = "SELECT emplnumber, lname, fname, address, address2, city, state, zipcode FROM vetcliniccorp.employee WHERE status = \"D\" ORDER BY lname, fname";
		break;
	case "04":
		$sql1 = "SELECT emplnumber, lname, fname, address, address2, city, state, zipcode, status FROM vetcliniccorp.employee ORDER BY lname, fname";
		break;
	case "05":
		$namestarts = $_POST["namestarts"];
		$sql1 = "SELECT emplnumber, lname, fname, address, address2, city, state, zipcode FROM vetclinic.client WHERE lname = ".$namestarts."% ORDER BY lname, fname";
		break;
}
$mysqlic = new mysqli('localhost', $_SESSION["user"], mc_decrypt($_SESSION["up"], ps_key), '');
$emplnumber = $_SESSION["employeenumber"];
$sql = "SELECT `sk27`, `sk32` FROM `vetcliniccorp`.`seckeys` WHERE `emplnumber` = $emplnumber and `sequence` = 1;";
