...
if ( 'new' == $editpetnum ) { // insert
	$sql = "INSERT INTO `vetclinic`.`pet` (`petname`, `petdob`, `petbreed`, `petgender`, `petfixed`, `petcolor`, `petdesc`, `license`,";
	...
	$sql = "UPDATE `vetclinic`.`pet` SET `petname` = \"".$petname."\", `petdob` = \"".$petdob."\", `petbreed` = \"".$petbreed."\",";
...
if ($client1 <> "") {
	$sql = "REPLACE INTO `vetclinic`.`clientpet` VALUES (" . $client1 . "," . $editpetnum . ")";
	$result = $mysqli->query($sql);
...
if ($client2 <> "") {
	$sql = "REPLACE INTO `vetclinic`.`clientpet` VALUES (" . $client2 . "," . $editpetnum . ")";
	$result = $mysqli->query($sql);
