...
if ($editpetnum == "")
{
     ...
     $sql = "SELECT * FROM `vetclinic`.`pet` WHERE `petnumber` = ".$editpetnum;
     ...
     <td class="center">
         <select id="breedFilter" size="3" onchange="applyBreedFilter(this);">
             <option value="all" selected>All Species</option>
              <?php
               $sql = "SELECT * FROM `vetclinic`.`code_species` ORDER BY `speciesdesc`;";
               $result = $mysqli->query($sql);
               $speciesCodes = array();
     ...
     <td class="field">
         <select id="petbreed" name="petbreed" size="5">
          ...
          <?php
               ...
               $sql = "SELECT * FROM `vetclinic`.`code_breed` ORDER BY `breeddesc`;";
               $result = $mysqli->query($sql);
...
if ( 'new' != $editpetnum ) {
     $sqlclient="SELECT clientnumber FROM `vetclinic`.`clientpet` WHERE `petnumber` = " . $editpetnum ;
