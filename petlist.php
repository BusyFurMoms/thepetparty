...
{
	case "01":
		$sql1 = "SELECT petnumber, petname, petbreed, petgender, petcolor, petdesc FROM `vetclinic`.`pet` WHERE status = \"A\" ORDER BY petname;";
		break;
	case "02":
		$sql1 = "SELECT petnumber, petname, petbreed, petgender, petcolor, petdesc FROM `vetclinic`.`pet` WHERE status = \"I\" ORDER BY petname;";
		break;
	case "03":
		$sql1 = "SELECT petnumber, petname, petbreed, petgender, petcolor, petdesc FROM `vetclinic`.`pet` WHERE status = \"D\" ORDER BY petname;";
		break;
	case "04":
		$sql1 = "SELECT petnumber, petname, petbreed, petgender, petcolor, petdesc FROM `vetclinic`.`pet` ORDER BY petname;";
		break;
	case "05":
		$namestarts = $_POST["namestarts"];
		$sql1 = "SELECT petnumber, petname, petbreed, petgender, petcolor, petdesc FROM `vetclinic`.`pet` WHERE petname LIKE \"".$namestarts."%\" ORDER BY petname;";
		break;
		}
...
for ($i = 0; $i < $row_cnt; $i++) {
	...
	if ($sk22 == "Y") {
     ...
     $sql2 = "SELECT `speciesdesc` FROM `vetclinic`.`code_species` WHERE `speciescode` = \"".$species."\";";
     ...
     $row1 = $row1.$rows[0]." ";
     $sql2 = "SELECT breeddesc FROM `vetclinic`.`code_breed` WHERE breedcode = \"".$row[2]."\";";
     $result = $mysqli->query($sql2);
     if ($result == TRUE) {
     } else {
          put_errormsg("Error getting species from code_species" . $mysqli->error);
