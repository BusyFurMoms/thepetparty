...
if($procdb == "P") {
	$sql = "SELECT * FROM `vetclinicproc`.`procedures` WHERE `procstatus` = 'A';";
	$result = $mysqli->query($sql);
...
$mysqli = new mysqli('localhost', $_SESSION["user"], mc_decrypt($_SESSION["up"], ps_key), '');
$sql = "SELECT * FROM `vetclinic`.`clientpet` WHERE `clientnumber` = ".$client.";";
$result = $mysqli->query($sql);
if ($result == FALSE)
{
...
for ($i = 0; $i < $row_cnt; $i++) {
	$row = $result->fetch_row();
	$sql = "SELECT * FROM `vetclinic`.`pet` WHERE `petnumber` = ".$row[1].";";
	$result2 = $mysqli->query($sql);
	if ($result2 == TRUE) {
		$row2 = $result2->fetch_row();
		$species = substr($row2[3], 0, 1);
		$sql = "SELECT `speciesdesc` FROM `vetclinic`.`code_species` WHERE `speciescode` = \"".$species."\";";
