...
if(isset($_GET["del"])) {
	$delete = $_GET["del"];
	$sql = "DELETE FROM `vetclinicmsgs`.`phonemsgs` WHERE `messagenumber` = ".$delete.";";
	$result = $mysqli->query($sql);
}
$employeenumber = $_SESSION["employeenumber"];
$sql = "SELECT * FROM `vetclinicmsgs`.`phonemsgs` WHERE  `emplnumber` = '$employeenumber';";
$result = $mysqli->query($sql);
if ($result == FALSE)
{
     $mysqli->close();
     put_errormsg("There are no remaining Messages (1)");
     redirect($scriptname);
     exit();
}
...
if ($row_cnt == 0) {
     $mysqli->close();
     put_errormsg("There are no remaining Messages (2)");
     redirect($scriptname);
     exit();
}
