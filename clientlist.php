...
{
	case "01":
		$sql1 = "SELECT clientnumber, lname, fname, address, address2, city, state, zipcode, email FROM `vetclinic`.`client` WHERE status = \"A\" ORDER BY lname, fname";
		break;
	case "02":
		$sql1 = "SELECT clientnumber, lname, fname, address, address2, city, state, zipcode, email FROM `vetclinic`.`client` WHERE status = \"I\" ORDER BY lname, fname";
		break;
	case "03":
		$sql1 = "SELECT clientnumber, lname, fname, address, address2, city, state, zipcode, email FROM `vetclinic`.`client` WHERE status = \"D\" ORDER BY lname, fname";
		break;
	case "04":
		$sql1 = "SELECT clientnumber, lname, fname, address, address2, city, state, zipcode, email, status FROM `vetclinic`.`client` ORDER BY lname, fname";
		break;
	case "05":
		$namestarts = $_POST["namestarts"];
		$sql1 = "SELECT clientnumber, lname, fname, address, address2, city, state, zipcode, email FROM `vetclinic`.`client` WHERE lname LIKE \"".$namestarts."%\" ORDER BY lname, fname";
		break;
	case "06":
		$namesounds = $_POST["namesounds"];
		$soundex = soundex($namesounds);
		$sql1 = "SELECT clientnumber, lname, fname, address, address2, city, state, zipcode, email FROM `vetclinic`.`client` WHERE soundex = ";
...
$sql = "SELECT `sk21` FROM `vetcliniccorp`.`seckeys` WHERE `emplnumber` = $emplnumber and `sequence` = 1;";
