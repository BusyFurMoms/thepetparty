...
$usePetInfo = ( !empty($petName) || !empty($petLicense) || !empty($petMicrochip) || !empty($petRabies) || !empty($petTattoo) );
    ...
    // We have data in both areas, or at least petInfo. We should not have any pet data without a client!!
        $sql = "SELECT lname, phonenumber, zipcode, petname, license, microchip, rabiestag, tattoonumber,
                       vetclinic.clientpet.clientnumber, vetclinic.clientpet.petnumber
                FROM vetclinic.clientpet JOIN vetclinic.client USING (clientnumber)
                LEFT JOIN vetclinic.pet USING(petnumber)
                LEFT JOIN vetclinic.clientphone USING (clientnumber)
        ...
        $sql = "SELECT lname, phonenumber, zipcode, petname, license, microchip, rabiestag, tattoonumber,
                       vetclinic.clientpet.clientnumber, vetclinic.clientpet.petnumber
                FROM vetclinic.client JOIN vetclinic.clientphone USING (clientnumber)
                LEFT JOIN vetclinic.clientpet USING (clientnumber)
                LEFT JOIN vetclinic.pet USING(petnumber)
                WHERE lname='" . $clientLast . "' or
                      phonenumber='" . $clientPhone . "' or
                      zipcode='" . $clientZip . "'";
