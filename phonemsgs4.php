...
$msgnum = $_GET["msg"];
$sql = "SELECT * FROM `vetclinicmsgs`.`phonemsgs` WHERE `messagenumber` = '".$msgnum."';";
...
$sql = "SELECT `lname`, `fname` FROM `vetcliniccorp`.`employee` WHERE `emplnumber` = '".$row[1]."';";
$result = $mysqli->query($sql);
if ($result == FALSE)
{
...
<br>
<?php
$sql = "UPDATE `vetclinicmsgs`.`phonemsgs` SET `read` = 'Y' WHERE `messagenumber` = '".$msgnum."';";
$result = $mysqli->query($sql);
$mysqli->close();
$ret = $_GET["ret"];
echo "<center>";
echo "<form method='post' action='phonemsgs3.php?ret=".$ret."&del=".$msgnum."'><input type='submit' value='Delete'></form>";
