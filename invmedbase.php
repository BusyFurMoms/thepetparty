...
if($u=="u") {
	...
	$sql = "SELECT `medid`,`meddesc` FROM `vetclinicinv`.`invmedicine` WHERE `status` = 'A';";
...
if($u = "y") {
	...
	$mysqli = new mysqli('localhost', $_SESSION["user"], mc_decrypt($_SESSION["up"], ps_key), '');
	$sql = "SELECT * FROM `vetclinicinv`.`invmedicine` WHERE `medid` = ".$baserec.";";
	$result = $mysqli->query($sql);
	if ($result == FALSE) {
...
<?php
$mysqli = new mysqli('localhost', $_SESSION["user"], mc_decrypt($_SESSION["up"], ps_key), '');
$sql = "SELECT `vendorid`,`vendorshortname` FROM `vetclinicinv`.`vendor` WHERE `vendorstatus` = 'A';";
$result = $mysqli->query($sql);
if ($result == FALSE)
{
